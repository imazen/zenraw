...
- Versioned public-API surface snapshot: `docs/public-api/zenraw.txt`, regenerated by `tests/public_api_doc.rs` on every `cargo test` (`ZEN_API_DOC=check` verified in CI's clippy job, `=off` elsewhere). `just api-doc` / `api-doc-check` recipes added; `just fmt` regenerates.
